...
export const validateBookStatus = (
  req: Request,
  res: Response,
  next: NextFunction
): void => {
  ...
  if (bookStatus != "read" && bookStatus != "to-read" && bookStatus != "pending"){
    res.status(400).json({
      success: false,
      message: "Le statut du livre ne correspond pas a l'un des trois formats attendus.",
      error: "Validation échouée",
